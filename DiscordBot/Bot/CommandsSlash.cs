    ...
    public class CommandsSlash : ApplicationCommandModule
    {
        ...
        public async Task PlayCommand(InteractionContext ctx, [Option("searchterm", "Search Term")] string term)
        {
            ...
            if (userVoiceS == null)
            {
                await ctx.EditResponseAsync(new DiscordWebhookBuilder
                {
                    Content = //"```You cannot use this command while not being in a channel.```"
                        Parser.FromNumber(guild.Language).SlashNotInChannel().CodeBlocked()
                });
                return;
            }
            ...
            if (player == null)
            {
                await ctx.EditResponseAsync(new DiscordWebhookBuilder
                {
                    Content = Parser.FromNumber(guild.Language).NoFreeBotAccounts().CodeBlocked()
                });
                return;
            }
        ...
        public async Task SkipCommand(InteractionContext ctx, [Option("times", "Times to skip")] long times = 1)
        {
            ...
            await ctx.CreateResponseAsync(Parser.FromNumber(guild.Language).SlashSkipping((int) times).CodeBlocked());
        ...
        public async Task PauseCommand(InteractionContext ctx)
        {
            ...
            await ctx.CreateResponseAsync(Parser.FromNumber(guild.Language).SlashPausing().CodeBlocked());
            player.Pause();
        }
        ...
        public async Task Shuffle(InteractionContext ctx)
        {
            try
            {
                ...
                player.Shuffle();
                await ctx.CreateResponseAsync(Parser.FromNumber(guild.Language).ShufflingTheQueue().CodeBlocked());
            }
            catch (Exception e)
            {
        ...
        public async Task Remove(InteractionContext ctx, [Option("num", "Index to remove")] long num)
        {
            ...
            await ctx.CreateResponseAsync(Parser.FromNumber(guild.Language).RemovingItem(item.GetName()).CodeBlocked());
        }
        ...
        public class AddCommand : ApplicationCommandModule
        {
                ...
                string choice)
            {
                ...
                if (userVoiceS == null)
                {
                    await ctx.EditResponseAsync(
                        new DiscordWebhookBuilder().WithContent(Parser.FromNumber(guild.Language).SlashNotInChannel()
                            .CodeBlocked()));
                    return;
                }
                ...
                {
                    await ctx.EditResponseAsync(
                        new DiscordWebhookBuilder().WithContent("Error: Unable to find album.".CodeBlocked()));
                    return;
                }
                ...
                await ctx.EditResponseAsync(new DiscordWebhookBuilder().WithContent(
                    $"Added album \'{album.AlbumName} - {album.Artist}\' to the queue.".CodeBlocked()));
            }
                ...
                string choice)
            {
                ...
                {
                    await ctx.EditResponseAsync(
                        new DiscordWebhookBuilder().WithContent("Error: Unable to find song.".CodeBlocked()));
                    return;
                }
                ...
                await ctx.EditResponseAsync(new DiscordWebhookBuilder().WithContent(
                    $"Added \"{song.GetName(guild.ShowOriginalInfo)}\" to the queue.".CodeBlocked()));
            }
        }
        ...
        public class CommandsSettings : ApplicationCommandModule
        {
            ...
            private static ushort LanguageParser(Language language)
            {
            ...
            public class UserCommands : ApplicationCommandModule
            {
                    ...
                    Language lang)
                {
                    try
                    {
                        ...
                        settings.Language = Parser.FromNumber(LanguageParser(lang));
            ...
            public class GuildCommands : ApplicationCommandModule
            {
                    ...
                    Language lang)
                {
                    try
                    {
                        ...
                        settings.Language = LanguageParser(lang);
                        await ctx.CreateResponseAsync((lang switch
                        {
                    ...
                    BooleanEnum booleanEnum)
                {
                    try
                    {
                        var settings = await GuildSettings.FromId(ctx.Guild.Id);
                        settings.Normalize =
                            booleanEnum switch {BooleanEnum.Disabled => false, BooleanEnum.Enabled => true, _ => true};
                        await ctx.CreateResponseAsync((Parser.FromNumber(settings.Language) switch
                        {
                            English =>
