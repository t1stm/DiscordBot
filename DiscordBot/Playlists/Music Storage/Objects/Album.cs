...
namespace DiscordBot.Playlists.Music_Storage.Objects
{
    public class Album
    {
        ...
        public static Album FromM3U(string location)
        {
            ...
            var songs = locations.Select(r => databaseItems.FirstOrDefault(song => song.RelativeLocation?.EndsWith(r) ?? false));
            album.Songs = songs.ToArray();
            
            if (album.Songs.Any(r => r == null))
            {
                throw new Exception("A song from the playlist is null.");
            }
